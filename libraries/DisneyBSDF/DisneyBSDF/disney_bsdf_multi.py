...
def GTR2_aniso(NdotH, HdotX, HdotY, ax, ay):
    return 1 / (
        np.pi * ax * ay * ((HdotX / ax) ** 2 + (HdotY / ay) ** 2 + NdotH ** 2) ** 2
        + eps
    )
...
def smithG_GGX_aniso(NdotV, VdotX, VdotY, ax, ay):
    return 1 / (
        NdotV + ((VdotX * ax) ** 2 + (VdotY * ay) ** 2 + NdotV ** 2) ** 0.5 + eps
    )
...
def disney_bsdf(
    L: torch.Tensor, V: torch.Tensor, N: torch.Tensor, params: Dict[str, torch.Tensor]
) -> torch.Tensor:
    batch_num = len(N)
    ...
    X = torch.cross(torch.Tensor([[0, 1, 0]]).to(N.dtype).to(N.device).expand_as(N), N)
    ...
    Ctint = torch.where(Cdlum > 0, Cdlin / (Cdlum + eps), torch.ones_like(Cdlum))
    ...
    ret1 = ((1 / np.pi) * mix(Fd, ss, subsurface) * Cdlin + Fsheen) * (1 - metallic)
    ret2 = Gs * Fs * Ds
