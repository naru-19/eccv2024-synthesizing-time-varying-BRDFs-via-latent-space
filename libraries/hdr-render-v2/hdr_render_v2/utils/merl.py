...
class Merl:
    ...
    scale = [1.0 / 1500, 1.15 / 1500, 1.66 / 1500]
    ...
    def __init__(self, merl_file):
        ...
        with open(merl_file, "rb") as f:
            ...
            length = (
                self.sampling_theta_h
                * self.sampling_theta_d
                * self.sampling_phi_d
            )
            if n[0] * n[1] * n[2] != length:
                raise IOError("Dimensions do not match")
            self.brdf = struct.unpack_from(
    ...
    def convert_to_fullmerl(self):
        if self.sampling_phi_d == 180:
            brdf = np.array(self.brdf).reshape(
                3,
                self.sampling_theta_h,
                self.sampling_theta_d,
                self.sampling_phi_d,
    ...
    def to_dataframe(self, mode="interpolation"):
        ...
        if mode == "raw":
            _brdf = np.apply_along_axis(
                lambda row: self.eval_raw(*row), 1, cols
            )
        elif mode == "interpolation":
            _brdf = np.apply_along_axis(
                lambda row: self.eval_interp(*row), 1, cols
            )
    ...
    def eval_interp(self, theta_h, theta_d, phi_d):
        ...
        idx_th_p = np.where(
            idx_th_p < self.sampling_theta_h - 1,
            idx_th_p,
            self.sampling_theta_h - 2,
        )
        idx_td_p = np.where(
            idx_td_p < self.sampling_theta_d - 1,
            idx_td_p,
            self.sampling_theta_d - 2,
        )
            ...
            for itd, wtd in zip(idx_td, weight_td):
                for ipd, wpd in zip(idx_pd, weight_pd):
                    ret_val = [
                        r + x * wth * wtd * wpd
                        for r, x, in zip(
                            ret_val, self.__eval_idx(ith, itd, ipd)
                        )
                    ]
    ...
    def __eval_idx(self, ith, itd, ipd):
        ...
        stride = (
            self.sampling_theta_h * self.sampling_theta_d * self.sampling_phi_d
        )

        ret = []
        for color in range(0, 3):
            ret += [
                self.brdf_np[(ind + color * stride).astype(np.int64)]
                * self.scale[color]
            ]
